...
- A pannable 980x1480 room with generated raster room and sprite assets.
...
Visual assets are generated by [scripts/generate-synthetic-assets.mjs](scripts/generate-synthetic-assets.mjs) using `pngjs`. The script creates deterministic PNGs for:

- `assets/rooms/bedroom-cartoon.png`
- `assets/messes/*.png`
- `assets/tools/*.png`
- `assets/characters/mother-cartoon.png`

Run this before static export when changing procedural art:

```bash
npm run generate:assets
```

The current approach is reproducible and deploy-safe. For a larger quality jump, the best next library/tooling choice is an AI image generation pipeline for the room background and transparent sprite cutouts, or a 2D art pipeline such as Krita/Photoshop plus TexturePacker. The code already consumes normal PNGs through `AssetRegistry`, so hand-painted or generated replacement assets can be swapped in without changing gameplay logic.
...
- `scripts/generate-synthetic-assets.mjs`: generates the current room, mess, tool, character, and audio placeholder assets.
...
- Visuals are more detailed generated 2D raster art, but still not final production illustration or photoreal assets.
- Audio is generated placeholder sound design.
